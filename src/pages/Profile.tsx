  ...
  if (!user || isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-black text-green-400 font-mono">
        <p className="text-xl mb-4 animate-pulse">🔄 Loading your Profile...</p>
  ...
  return (
    <div className="min-h-screen w-full bg-black text-green-300 font-mono px-4 py-12">
      <div className="max-w-4xl mx-auto space-y-10">
        {/* Profile Card */}
        <div className="bg-gradient-to-br from-red-800 via-black to-red-800 rounded-3xl p-8 border border-green-800 shadow-lg shadow-green-500/10 hover:shadow-green-400/20 transition-all duration-300">
          <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <img
                src={
                  userData?.photoURL && userData.photoURL !== ""
                    ? userData.photoURL
                    : "https://ui-avatars.com/api/?name=" + (userData?.displayName || "User")
                }
                alt="Profile"
            ...
            <div className="text-center md:text-left space-y-2">
              <h1 className="text-4xl font-bold tracking-wide text-lime-300 animate-pulse">
                {userData?.displayName || "Unnamed Hacker"}
              </h1>
              <p className="text-sm text-green-400">{userData?.email}</p>
              <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-2">
                <span
                  className={`px-3 py-1 bg-green-900/50 border border-green-400 text-xs rounded-full ${getRoleColor(
                    userData?.role
                  )}`}
                >
